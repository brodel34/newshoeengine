...
function wcmvp_render_most_viewed_products( $num_posts = 10 ) {
	...
	if ( $r->have_posts() ) {
		...
		while ( $r->have_posts() ) {
			...
			?>
			<a 
			href="<?php echo esc_url( get_permalink( $product->id ) ); ?>"
			>
			<div class="main-popular">
			<div class="popular product type-product status-publish has-post-thumbnail  product_cat-raffles product_cat-upcoming entry clearfix entry-grid entry-columns-6 thumbnail-color-tone-light entry-product entry-grid_overlay_quickview entry-product-grid_overlay_quickview instock shipping-taxable purchasable product-type-simple 6 aos-init aos-animate">
			<?php echo $product->get_image(); ?>
			
					
			<div class="product-thumbnail-container-popular">	
				
			<?php echo $product->get_title(); ?>		
			
			
			</div>
				<div class="product-summary clearfix margin0">
					
				
				<span class="woocommerce-Price-amount amount"><?php echo $product->get_price_html(); ?></span>
				<div class="single-add-to-wishlist heart_populars">
				</a>
