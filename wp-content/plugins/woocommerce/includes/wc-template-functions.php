...
if ( ! function_exists( 'woocommerce_catalog_ordering' ) ) {
	...
	function woocommerce_catalog_ordering() {
		...
		$catalog_orderby_options = apply_filters( 'woocommerce_catalog_orderby', array(
			 'menu_order' => __( 'Trending', 'woocommerce' ),
			// 'popularity' => __( 'Sort by popularity', 'woocommerce' ),
			// 'rating'     => __( 'Sort by average rating', 'woocommerce' ),
			'date'       => __( 'Newness', 'woocommerce' ),
			// 'views'       => __( 'Sort by views', 'woocommerce' ),
	...
	function woocommerce_default_product_tabs( $tabs = array() ) {
		...
		if ( comments_open() ) {
			if($product->get_review_count()){
				$countssss = $product->get_review_count();
			}else{
				$countssss = "0";
			}
			$tabs['reviews'] = array(
				/* translators: %s: reviews count */
				'title'    => sprintf( __( 'Reviews (%d)', 'woocommerce' ), $countssss ),
				'priority' => 30,
...
if ( ! function_exists( 'woocommerce_breadcrumb' ) ) {
	...
	function woocommerce_breadcrumb( $args = array() ) {
		$args = wp_parse_args( $args, apply_filters( 'woocommerce_breadcrumb_defaults', array(
			...
			'before'      => '',
			'after'       => '',
			'home'        => _x( 'Shoe Engine', 'breadcrumb', 'woocommerce' ),
		) ) );
