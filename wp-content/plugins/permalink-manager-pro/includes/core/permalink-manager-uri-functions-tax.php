...
class Permalink_Manager_URI_Functions_Tax extends Permalink_Manager_Class {
	...
	*/
	public function edit_uri_box($term = '') {
		...
		$is_pro_active = Permalink_Manager_Admin_Functions::is_pro_active();

		// A. New term
		if(empty($term->term_id)) {
			$html = "<div class=\"form-field\">";
			$html .= "<label for=\"term_meta[uri]\">{$label}</label>";
			$html .= ($is_pro_active) ? "<input type=\"text\" name=\"custom_uri\" id=\"custom_uri\" value=\"\">" : Permalink_Manager_Admin_Functions::pro_text();
		...
		// B. Edit term
			...
			$html .= "<td><div>";
			$html .= ($is_pro_active || $custom_uri) ? Permalink_Manager_Admin_Functions::display_uri_box($term) : Permalink_Manager_Admin_Functions::pro_text();
			$html .= "</div></td>";
			$html .= "</tr>";
		}
