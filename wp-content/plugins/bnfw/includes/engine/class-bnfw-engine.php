...
class BNFW_Engine {
	...
	public function send_notification( $setting, $id ) {
		...
		if ( ! $notification_disabled ) {
			...
			$product = $this->handle_shortcodes( "[product]", $setting['notification'], $id );
			$feed_type = $this->handle_shortcodes( "[feed_type]", $setting['notification'], $id );
			$feed_options = $this->handle_shortcodes( "[feed_options]", $setting['notification'], $id );
			$emails  = $this->get_emails( $setting, $id );
			$headers = $this->get_headers( $emails );
			if($feed_options == 'Regular' ){
				$subject = 'new '.$feed_type ;
			}elseif($feed_options == 'On Sale'){
                $subject = 'new Sale' ;
			}

			$wished_users = $wpdb->get_results( "SELECT author FROM wp_tinvwl_items WHERE product_id=$product" );
		
			foreach ($wished_users as $value) 
			$array[] = $value->author;
			$wished_users_email = [];
			$usersEmail=[];
			foreach ( $array as $key => $email_to){
				$wished_users_email = $wpdb->get_results( "SELECT user_email FROM wp_users WHERE id=$email_to" );
				$usersEmail[$key]=$wished_users_email[0]->user_email;
			}
			...
			$emails = apply_filters( 'bnfw_emails', $emails, $setting );
				...
				// $fp = fopen('/var/www/html/wp-content/plugins/bnfw/includes/engine/output.txt', 'w');		
				// fwrite($fp, print_r($subject, true) );
				// fclose($fp);

			if ( isset( $usersEmail ) && is_array( $usersEmail ) ) {
	...
	public function post_shortcodes( $message, $post_id ) {
		...
		$message = str_replace( '[product]', $post->product, $message );
		$message = str_replace( '[feed_type]', $post->feed_type, $message );
		$message = str_replace( '[feed_options]', $post->feed_options, $message );
