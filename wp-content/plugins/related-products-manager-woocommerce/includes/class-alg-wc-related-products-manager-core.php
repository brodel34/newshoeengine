...
if ( ! class_exists( 'Alg_Dummy_Term' ) ) {
	/**
	 * Alg_Dummy_Term class.
	 *
	 * @version 1.0.0
	 * @since   1.0.0
	 */
	class Alg_Dummy_Term {
		public $term_id;
...
if ( ! class_exists( 'Alg_WC_Related_Products_Manager_Core' ) ) :

class Alg_WC_Related_Products_Manager_Core {
	...
	function __construct() {
		...
		if ( 'yes' === get_option( 'alg_wc_related_products_manager_enabled', 'yes' ) ) {
			...
			// Relate by Category
			if ( 'no' === get_option( 'alg_wc_related_products_manager_relate_by_category', 'yes' ) ) {
			...
			// Relate by Tag
			if ( 'no' === get_option( 'alg_wc_related_products_manager_relate_by_tag', 'yes' ) ) {
			...
			if ( 'yes' === get_option( 'alg_wc_related_products_manager_hide', 'no' ) ) {
			...
			if ( 'yes' === apply_filters( 'alg_wc_related_products_manager', 'no', 'value_relate_per_product' ) ) {
				add_filter( 'woocommerce_related_products', array( $this, 'hide_related_per_product' ), PHP_INT_MAX, 3 );
			}

			// Exclude from Related
			if ( 'yes' === get_option( 'alg_wc_related_products_manager_exclude_section_enabled', 'no' ) ) {
				add_filter( 'woocommerce_related_products', array( $this, 'exclude_by_taxonomy' ), PHP_INT_MAX, 3 );
			}

			// Init finished
			do_action( 'alg_wc_related_products_manager_init_finished' );
	...
	function exclude_by_taxonomy( $related_posts, $product_id, $args ) {
		$terms_to_exclude = get_option( 'alg_wc_related_products_manager_exclude_taxonomy', array() );
	...
	function hide_related_per_product( $related_posts, $product_id, $args ) {
		return apply_filters( 'alg_wc_related_products_manager', $related_posts, 'hide_per_product', array( 'product_id' => $product_id ) );
	}
	...
	function related_products_args_wc3( $args ) {
		// Related Num
		$args['posts_per_page'] = get_option( 'alg_wc_related_products_manager_num', 3 );
		// Order By
		$args['orderby'] = get_option( 'alg_wc_related_products_manager_orderby', 'rand' );
		// Order
		if ( 'rand' != $args['orderby'] ) {
			$args['order'] = get_option( 'alg_wc_related_products_manager_order', 'desc' );
		}
		return $args;
	}
	...
	function output_related_products_args_wc3( $args ) {
		$args['columns'] = get_option( 'alg_wc_related_products_manager_columns', 3 );
	...
	function get_product_attribute_query( $args, $product_id ) {
		$attribute_name   = get_option( 'alg_wc_related_products_manager_relate_by_attribute_name',  '' );
		$attribute_value  = get_option( 'alg_wc_related_products_manager_relate_by_attribute_value', '' );
		if ( '' === $attribute_value ) {
			$attribute_value = apply_filters( 'alg_wc_related_products_manager', '', 'relate_by_product_attribute', array( 'product_id' => $product_id, 'attribute_name' => $attribute_name ) );
		}
		if ( 'global' === get_option( 'alg_wc_related_products_manager_relate_by_attribute_type', 'global' ) ) {
	...
	function get_related_products_ids_wc3( $product_id ) {
		...
		if ( 'yes' === apply_filters( 'alg_wc_related_products_manager', 'no', 'value_relate_per_product' ) && 'yes' === apply_filters( 'alg_wc_related_products_manager', 'no', 'value_relate_per_product_enabled', array( 'product_id' => $product_id ) ) ) {
			// Relate per Product (Manual)
			if ( '' != ( $related_per_product = apply_filters( 'alg_wc_related_products_manager', '', 'ids_relate_per_product', array( 'product_id' => $product_id ) ) ) ) {
				$include_ids = $related_per_product;
			}
		} elseif ( 'yes' === get_option( 'alg_wc_related_products_manager_relate_by_attribute_enabled', 'no' ) ) {
			$args = array(
				'post_type'      => 'product',
				'post_status'    => 'any',
				'posts_per_page' => -1,
	...
	function related_products_query_wc3( $_query, $product_id ) {
		...
		$override_cats_and_tags = get_option( 'alg_wc_related_products_manager_override_cats_and_tags', 'yes' );
		$include_ids = implode( ',', array_map( 'absint', $include_ids ) );
		$cats_array  = ( 'yes' === $override_cats_and_tags ? array() : apply_filters( 'woocommerce_product_related_posts_relate_by_category', true, $product_id ) ? apply_filters( 'woocommerce_get_related_product_cat_terms', wc_get_product_term_ids( $product_id, 'product_cat' ), $product_id ) : array() ) ;
		$tags_array  = ( 'yes' === $override_cats_and_tags ? array() : apply_filters( 'woocommerce_product_related_posts_relate_by_tag', true, $product_id ) ? apply_filters( 'woocommerce_get_related_product_tag_terms', wc_get_product_term_ids( $product_id, 'product_tag' ), $product_id ) : array() );
		$_product    = wc_get_product( $product_id );
		$exclude_ids = array_merge( array( 0, $product_id ), $_product->get_upsell_ids() );
		if ( 'yes' === get_option( 'alg_wc_related_products_manager_hide', 'no' ) ) {
			$limit = 0;
		} else {
			$limit = get_option( 'alg_wc_related_products_manager_num', 3 );
			$limit = $limit > 0 ? $limit : 5;
			$limit += get_option( 'alg_wc_related_products_manager_limit', 20 );
		}
		//////////////////////////////////////////////////////////////////////
	...
	function related_products_args( $args ) {
		...
		$args['posts_per_page'] = get_option( 'alg_wc_related_products_manager_num', 3 );
		// Order By
		$orderby = get_option( 'alg_wc_related_products_manager_orderby', 'rand' );
		$args['orderby'] = $orderby;
		if ( 'meta_value' === $orderby || 'meta_value_num' === $orderby ) {
			$args['meta_key'] = get_option( 'alg_wc_related_products_manager_orderby_meta_value_meta_key', '' );
		}
		// Order
		if ( get_option( 'alg_wc_related_products_manager_orderby', 'rand' ) != 'rand' ) {
			$args['order'] = get_option( 'alg_wc_related_products_manager_order', 'desc' );
		}
		// Change Related Products
	...
	function fix_empty_initial_related_products( $terms, $product_id ) {
		if (
			( 'yes' === get_option( 'alg_wc_related_products_manager_relate_by_attribute_enabled', 'no' ) ) ||
			(
				'yes' === apply_filters( 'alg_wc_related_products_manager', 'no', 'value_relate_per_product' ) &&
				'yes' === apply_filters( 'alg_wc_related_products_manager', 'no', 'value_relate_per_product_enabled', array( 'product_id' => $product_id ) ) &&
				'' != apply_filters( 'alg_wc_related_products_manager', '', 'ids_relate_per_product', array( 'product_id' => $product_id ) )
			)
		) {
			add_filter( 'woocommerce_product_related_posts_relate_by_category', '__return_false', PHP_INT_MAX );
			add_filter( 'woocommerce_product_related_posts_relate_by_tag',      '__return_false', PHP_INT_MAX );
			if ( empty( $terms ) ) {
				$dummy_term = new Alg_Dummy_Term();
				$terms[] = $dummy_term;
			}
		}
		return $terms;
	...
	function related_products_columns( $columns ) {
		return get_option( 'alg_wc_related_products_manager_columns', 3 );
	}
...
return new Alg_WC_Related_Products_Manager_Core();
