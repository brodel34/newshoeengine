...
if ( ! class_exists( 'Alg_WC_Related_Products_Manager_Settings_Per_Product' ) ) :

class Alg_WC_Related_Products_Manager_Settings_Per_Product {
	...
	function __construct() {
		$this->id = 'per_product';
		if ( 'yes' === get_option( 'alg_wc_related_products_manager_enabled', 'yes' ) && 'yes' === apply_filters( 'alg_wc_related_products_manager', 'no', 'value_relate_per_product' ) ) {
	...
	function get_meta_box_options() {
		$is_chosen_select = ( 'chosen_select' === get_option( 'alg_wc_related_products_manager_relate_per_product_select_type', 'chosen_select' ) );
		$product_id       = get_the_ID();
		$products         = $this->get_products( array(), 'publish' );
		unset( $products[ $product_id  ] );
		$options = array(
			array(
				'name'       => 'alg_wc_related_products_manager_enabled',
				'default'    => 'no',
				'type'       => 'select',
				'options'    => array(
			...
			array(
				'name'       => 'alg_wc_related_products_manager_ids',
				'default'    => '',
				'type'       => 'select',
				'options'    => $products,
			...
			array(
				'name'       => 'alg_wc_related_products_manager_cat_ids',
				'default'    => '',
				'type'       => 'select',
				'options'    => alg_wc_related_products_manager()->settings['general']->get_terms( array( 'taxonomy' => 'product_cat', 'orderby' => 'name', 'hide_empty' => false ) ),
			...
			array(
				'name'       => 'alg_wc_related_products_manager_tag_ids',
				'default'    => '',
				'type'       => 'select',
				'options'    => alg_wc_related_products_manager()->settings['general']->get_terms( array( 'taxonomy' => 'product_tag', 'orderby' => 'name', 'hide_empty' => false ) ),
	...
	function save_meta_box( $post_id, $post ) {
		...
		if ( ! isset( $_POST[ 'alg_wc_related_products_manager_' . $this->id . '_save_post' ] ) ) {
			return;
		}
		// Save options
	...
	function add_meta_box() {
		...
		add_meta_box(
			'alg_wc_related_products_manager_' . $this->id,
	...
	function create_meta_box() {
		...
		$html .= '</table>';
		$html .= '<input type="hidden" name="alg_wc_related_products_manager_' . $this->id . '_save_post" value="alg_wc_related_products_manager_' . $this->id . '_save_post">';
		echo $html;
	}
...
return new Alg_WC_Related_Products_Manager_Settings_Per_Product();
