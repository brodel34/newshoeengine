 ...
 */
/* jshint -W062 */
/* global PrequelleParams, WOW, AOS, WVC, Cookies, Event, objectFitImages, WVCBigText, WVCParams, PrequelleYTVideoBg, Vimeo */
	...
	return {
		...
		toggleMenu : function () {
			$( document ).on( 'click', '.toggle-mobile-menu', function( event ) {
				...
				$( window ).trigger( 'prequelle_mobile_menu_toggle_button_click', [ $( this ) ] );
				$( 'body' ).toggleClass( 'mobile-menu-toggle' );
			} );

			$( document ).on( 'click', '.toggle-side-panel', function( event ) {
				event.preventDefault();
				$( window ).trigger( 'prequelle_side_panel_toggle_button_click', [ $( this ) ] );
				$( 'body' ).toggleClass( 'side-panel-toggle' );
			} );

			$( document ).on( 'click', '.toggle-offcanvas-menu', function( event ) {
				event.preventDefault();
				$( window ).trigger( 'prequelle_offcanvas_menu_toggle_button_click', [ $( this ) ] );
			...
			$( document ).on( 'click', '.toggle-lateral-menu', function( event ) {
				event.preventDefault();
				$( window ).trigger( 'prequelle_lateral_menu_toggle_button_click', [ $( this ) ] );
		...
		flexSlider : function () {
			...
			if ( $.isFunction( $.flexslider ) ) {
				...
				$( '.entry-slider' ).flexslider( {
					animation: PrequelleParams.entrySliderAnimation,
					//controlNav: false,
					//directionNav: false,
					slideshow : true,
					//pauseOnHover : false,
		...
		lightbox : function () {
			...
			if ( 'fancybox' === PrequelleParams.lightbox ) {
				...
				$( selectors ).fancybox( PrequelleParams.fancyboxSettings );
				...
				$( '.gallery-quickview, .wvc-gallery-quickview' ).unbind().on( 'click', function() {
					...
					$.fancybox.open( quickviewData, PrequelleParams.fancyboxSettings );
					return false;
				} );
		...
		videoThumbnailPlayOnHover : function() {
			...
			$( 'iframe.youtube-bg', itemsContainer ).each( function() {
				if ( 'undefined' !== typeof PrequelleYTVideoBg && PrequelleYTVideoBg.players[ $( this ).data( 'youtube-video-id' ) ] ) {
					PrequelleYTVideoBg.players[$( this ).data( 'youtube-video-id' )].pauseVideo();
				} else {
				...
				if ( $iframe.length ) {
					...
					if ( $iframe.hasClass( 'youtube-bg' ) ) {
						...
						$( this ).on( 'mouseenter', function() {
							//console.log( 'enter' );
							if ( 'undefined' !== typeof PrequelleYTVideoBg && PrequelleYTVideoBg.players[ YTPlayerId ] ) {
								PrequelleYTVideoBg.players[YTPlayerId].playVideo();
							} else{
						...
						$( this ).on( 'mouseleave', function() {
							...
							setTimeout( function() {
								if ( 'undefined' !== typeof PrequelleYTVideoBg && PrequelleYTVideoBg.players[ YTPlayerId ] ) {
									PrequelleYTVideoBg.players[YTPlayerId].pauseVideo();
								} else {
									//this.contentWindow.postMessage( '{"event":"command","func":"' + 'pauseVideo' + '", "args":""}', '*' );
								}
							}, 500 );
